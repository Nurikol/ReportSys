...
(function( $ ) {
	...
	var initCalendarDragNDrop = function() {
		...
		var initCalendar= function() {
			var $calendar = $('#studentDailyCalendar');
			var date = new Date();
			var d = date.getDate();
			var m = date.getMonth();
			...
			$calendar.fullCalendar({
				...
				drop : function(date, allDay) { // this function is called when
					...
					copiedEventObject.className = $externalEvent
							.attr('data-event-class');
				...
				events : function(start, end, timezone, callback) {
					...
					$.ajax({
						url : 'statistics/getReportList/studentDaily',
						type : 'POST',
		...
		$calendarButtons
			.filter('.fc-button-prev, .fc-button-today, .fc-button-next')
				.wrapAll('<div class="btn-group mt-sm mr-md mb-sm ml-sm" style="width:110px"></div>')
				.parent()
